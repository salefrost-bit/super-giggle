...
| `sessions` | `id`, `user_id` (NOT NULL — sesija se kreira samo za ulogovane, gost nikad ne piše u bazu), `difficulty_level_id`, `total_cards`, `rep_multiplier`, `game_mode` (default `'classic'`), `settings` (JSONB), `started_at`, `completed_at`, `total_duration_seconds`, `status` | `game_mode` i `settings` su prošireni bez menjanja kolona kad se doda npr. `'challenge'` mod u Fazi 2. `difficulty_level_id` se čuva da bi istorija treninga (sekcija 7) mogla da prikaže težinu. |
